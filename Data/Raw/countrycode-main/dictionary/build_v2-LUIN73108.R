...
datasets <- datasets[datasets != './countrycode-main/dictionary/data_iso_ret.csv']
...
##################################################################
##################################################################
###### countrycode and codelist_panel  -- MOVE TO \build_v2.r\
##################################################################

getwd()
library(tidyverse)

codelist_panel <- codelist_panel %>% 
  select(year, starts_with("count"), iso3n, iso3c, cown, cowc, un, wb, gwn, everything()) 

codelist_panel <- codelist_panel %>% arrange(country.name.en, year)


glimpse(codelist_panel)
library(janitor)
cc <- clean_names(codelist_panel)
glimpse(cc)
names(cc)

cc50 <- cc %>% filter(year>=1950)
names(cc50)
unique(cc50$country_name_en)

library(panelView)
cc %>% filter(year>1900 &  (country_name_en == 'Armenia' |  country_name_en == 'Yugoslavia')) %>% 
  panelview( cown ~1, index = c("country_name_en", "year"),  type = 'miss', axis.adjust = TRUE)

cc %>% filter(year>1800 &  (country_name_en == 'Panama' |  country_name_en == 'Germany')) %>% 
  panelview( cown ~1, index = c("country_name_en", "year"),  type = 'miss', axis.adjust = TRUE)

is.pconsecutive(pp)

library(panelView)
cc %>% filter(country_name_en == 'Yemen' |  country_name_en == 'Yugoslavia') %>% 
  panelview( year ~1, index = c("country_name_en", "year"),  type = 'miss', axis.adjust = TRUE)

cc %>% filter(country_name_en == 'Yemen') %>% {sort(unique(.$year))}
