...
iso3cRet_country.name.enCustom <- c( "CSK" = "Czechoslovakia", "SCG" = "Serbia and Montenegro", "DDR" = "Germany", "YUG" = "Yugoslavia")
...
sort(unique(ccpConBal50$cown))
names(ineqlong)
guess_field(ineqlong$c.n.en0_UTIP)
guess_field(ineqlong$code_UTIP)
guess_field(ineqlong$country_UTIP)
guess_field(ineqlong$countryname_UTIP)
guess_field(ineqlong$cown_UTIP)
guess_field(ccpConBal50$cown)

names(ineqlong)
mmineqlong50 <- left_join(ccpConBal50, ineqlong,  by = c("country.name.en" ="c.n.en0_UTIP", "year" = "year_UTIP") )
names(mmineqlong50)

dim(ccpConBal50)
dim(ineqlong)
dim(mmineqlong50)  ## still too many rows in the merged data
names(ccpConBal50)
names(mmineqlong50)
...
mmineqlong50[which( (duplicated(df) | duplicated(df, fromLast = TRUE) ) ),] %>% View()
...
mmineqlong50[which( !(
  (duplicated(df) | duplicated(df, fromLast = TRUE) ) & (mmineqlong50$countryname_UTIP == "Serbia and Montenegro")
   )
  ),] %>% View()

mmUTIP50 <- mmineqlong50[which( !(
  (duplicated(df) | duplicated(df, fromLast = TRUE) ) & (mmineqlong50$countryname_UTIP == "Serbia and Montenegro")
)
),]

save(mmUTIP50, file = "mmUTIP50.RData")

getwd()

library(haven)
library(janitor)
write_dta(clean_names(mmUTIP50), path = "mmUTIP50.dta" )
...
cow_states <- read.csv("./COW/StateSystemMembership/states2016.csv")
cow_majors <- read.csv("./COW/StateSystemMembership/majors2016.csv")
cow_systems <- read.csv("./COW/StateSystemMembership/system2016.csv")
...
mmCOW50 <- left_join(ccpConBal50, mmf50, by = c("cown" = "ccode_COW", "year"="year_COW") )
names(mmCOW50)
...
mmPWT50 <- left_join(ccpConBal50, pwt10, by = c("iso3c" = "isocode_PWT", "year"="year_PWT") )
names(mmPWT50)
...
library(janitor)
library(haven)
write_dta(clean_names(mmSWIID50), path = "mmSWIID50.dta")

getwd()
save(mmSWIID50, file = "mmSWIID50.RData")
